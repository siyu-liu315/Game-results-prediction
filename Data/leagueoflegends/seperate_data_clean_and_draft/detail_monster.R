...
for (n in game){
  x = 0
  match <- earth_dragon  %>% filter(Address == n)
  row = nrow(match)
  for (i in 1:60) { 
    if (i %in% match$Time_int[1:row])
    {
...
df_earth_dragon <- data.frame(matchname = matchname,
                              min = min,
                              earth_dragon = type)

write.csv(df_earth_dragon,file = "clean_earth_dragon.csv")
...
for (n in game){
  x = 0
  match <- fire_dragon  %>% filter(Address == n)
  row = nrow(match)
  for (i in 1:60) { 
    if (i %in% match$Time_int[1:row])
    {
...
df_fire_dragon <- data.frame(matchname = matchname,
                             min = min,
                             fire_dragon = type)

write.csv(df_fire_dragon,file = "clean_fire_dragon.csv")
...
for (n in game){
  x = 0
  match <- water_dragon  %>% filter(Address == n)
  row = nrow(match)
  for (i in 1:60) { 
    if (i %in% match$Time_int[1:row])
    {
...
df_water_dragon <- data.frame(matchname = matchname,
                              min = min,
                              water_dragon = type)

write.csv(df_water_dragon,file = "clean_water_dragon.csv")
...
df_dragon <- data.frame(matchname = matchname,
                        min = min,
                        dragon = type)

write.csv(df_dragon,file = "clean_dragon.csv")
